...
class SquareDriveNode(Node):
    ...
    def publish_velocity(self, linear=0.0, angular=0.0):
        ...
        if self.counter > 10:
            self.counter = 0
            self.get_logger().info(f"st: {self.state} Pos: ({self.current_x:.2f}, {self.current_y:.2f}), Yaw: {self.current_yaw:.2f} rad")
    ...
    def save_starting_odom(self):
        ...
        while rclpy.ok() and (self.current_x == 0.0 and self.current_y == 0.0 and self.current_yaw == 0.0):
    ...
    def calculate_cmd_vel_turn(self):
        ...
        if abs(angle_diff) > 0.1:
            self.linear_x = 0.0
            self.angular = 0.5 * angle_diff / abs(angle_diff)
        else:
            self.linear_x = 0.0
            self.angular = 0.0
            self.state = 'straight'
